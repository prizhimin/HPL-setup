#!/bin/sh
# Benchmark HPL для процессора Intel Core i7-6800K (6 ядер, 12 потоков)
...
# ОПТИМИЗАЦИЯ ДЛЯ i7-6800K (Broadwell-EP): Используем TARGET=HASWELL и разрешаем AVX2
make TARGET=HASWELL USE_OPENMP=1 NUM_THREADS=12
...
cd openmpi-5.0.8
# ОПТИМИЗАЦИЯ ДЛЯ i7-6800K: Изменены флаги на -march=haswell
CFLAGS="-O3 -march=haswell -mtune=haswell -funroll-loops -floop-optimize" ./configure --prefix=$HOME/opt/OpenMPI
make -j $(nproc)
make install
...
# Создание Makefile для i7-6800K
cat > Make.linux << 'EOF'
SHELL        = /bin/sh
CD           = cd
CP           = cp
...
CCNOOPT      = $(HPL_DEFS)
# ОПТИМИЗАЦИЯ ДЛЯ i7-6800K: Изменены флаги на -march=haswell
CCFLAGS      = $(HPL_DEFS) -fomit-frame-pointer -O3 -march=haswell -mtune=haswell -funroll-loops -floop-optimize
...
# Создание HPL.dat конфигурации для i7-6800K (6 ядер / 12 потоков)
# Рекомендация по N: sqrt(Общая память * 0.8) * 1024^2 / 8 | Для 32ГБ ~ 42000
...
3            Ps  # P x Q = 3 x 4 = 12 процессов (все логические ядра)
4            Qs
16.0         threshold
1            # of panel fact
...
1            DEPTHs (>=0)  # Можно поэкспериментировать с 0 или 1
...
echo "Запуск HPL теста на i7-6800K (6 ядер / 12 потоков)..."

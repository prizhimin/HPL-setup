#!/bin/sh
# Установка HPL для Intel i9-10850K (Comet Lake) с 32 ГБ ОЗУ

echo "Установка HPL для Intel i9-10850K (Comet Lake) с 32 ГБ ОЗУ..."
...
# ОПТИМИЗАЦИЯ для архитектуры Comet Lake
make TARGET=HASWELL USE_OPENMP=1 NUM_THREADS=20
...
cd openmpi-5.0.8
# ОПТИМИЗАЦИЯ: Флаги для Comet Lake с AVX2
CFLAGS="-O3 -march=skylake -mtune=skylake -mavx2 -mfma -funroll-loops" ./configure --prefix=$HOME/opt/OpenMPI
make -j 10
make install
...
# Создание Makefile для i9-10850K (Comet Lake)
cat > Make.cometlake << 'EOF'
SHELL        = /bin/sh
CD           = cd
CP           = cp
LN_S         = ln -s
...
CC           = $(MPdir)/bin/mpicc
CCNOOPT      = $(HPL_DEFS)
CCFLAGS      = $(HPL_DEFS) -fomit-frame-pointer -O3 -march=skylake -mtune=skylake -mavx2 -mfma -funroll-loops
LINKER       = $(MPdir)/bin/mpif77
LINKFLAGS    = $(CCFLAGS) -Wl,--no-as-needed
...
# Создание HPL.dat конфигурации для i9-10850K с 32GB памяти
cat > bin/cometlake/HPL.dat << 'EOF'
...
echo "Бинарный файл: ~/hpl-2.3/bin/cometlake/xhpl"
echo "Конфигурационный файл: ~/hpl-2.3/bin/cometlake/HPL.dat"

cd ~/hpl-2.3/bin/cometlake

echo "Запуск HPL на i9-10850K с 32 ГБ ОЗУ..."

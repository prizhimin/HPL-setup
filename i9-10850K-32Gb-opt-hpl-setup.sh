#!/bin/sh
# HPL установка для Intel i9-10850K (Comet Lake) с 32 ГБ ОЗУ
...
cd openmpi-5.0.8
# ОПТИМИЗАЦИЯ: Улучшенные флаги для Comet Lake
CFLAGS="-O3 -march=skylake -mtune=skylake -mavx2 -mfma -funroll-loops -floop-unroll-and-jam -mprefer-vector-width=256" ./configure --prefix=$HOME/opt/OpenMPI
make -j 10
make install
...
# Создание Makefile для i9-10850K
cat > Make.linux << 'EOF'
SHELL        = /bin/sh
CD           = cd
CP           = cp
...
CCFLAGS      = $(HPL_DEFS) -fomit-frame-pointer -O3 -march=skylake -mtune=skylake -mavx2 -mfma -funroll-loops -floop-unroll-and-jam -mprefer-vector-width=256
...
echo "Запуск HPL теста на i9-10850K с 32GB памяти..."
echo "Используется 20 процессов (10 ядер + HT)"
cd ~/hpl/bin/linux

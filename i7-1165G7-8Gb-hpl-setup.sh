#!/bin/sh
# HPL установка для Intel i7-1165G7 (Tiger Lake) с 8 ГБ ОЗУ
...
# ОПТИМИЗАЦИЯ для Tiger Lake: AVX512 и современные инструкции
make TARGET=SKYLAKEX USE_OPENMP=1 NUM_THREADS=8
...
cd openmpi-5.0.8
# ОПТИМИЗАЦИЯ: Флаги для Tiger Lake с AVX512
CFLAGS="-O3 -march=tigerlake -mtune=tigerlake -mavx512f -mavx512dq -mavx512vl -mavx512bw -funroll-loops" ./configure --prefix=$HOME/opt/OpenMPI
make -j 4
make install
...
# Создание Makefile для i7-1165G7 (Tiger Lake)
cat > Make.tigerlake << 'EOF'
SHELL        = /bin/sh
CD           = cd
CP           = cp
LN_S         = ln -s
...
CCNOOPT      = $(HPL_DEFS)
# ОПТИМИЗАЦИЯ: Флаги для Tiger Lake с AVX512
CCFLAGS      = $(HPL_DEFS) -fomit-frame-pointer -O3 -march=tigerlake -mtune=tigerlake -mavx512f -mavx512dq -mavx512vl -funroll-loops
...
# Создание HPL.dat конфигурации для i7-1165G7 с 8GB памяти
cat > ~/hpl/bin/tigerlake/HPL.dat << 'EOF'
...
echo "Бинарный файл: ~/hpl/bin/tigerlake/xhpl"
echo "Конфигурационный файл: ~/hpl/bin/tigerlake/HPL.dat"

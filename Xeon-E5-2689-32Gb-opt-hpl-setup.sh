...
make TARGET=SANDYBRIDGE NO_AVX2=1 USE_OPENMP=1 NUM_THREADS=16
...
# -march=sandybridge: целевая архитектура Xeon E5-2689 (Sandy Bridge-EP)
# -mtune=sandybridge: оптимизация под конкретные особенности микроархитектуры
# -mavx: явное включение AVX инструкций (256-битные вектора)
# -O3: максимальный уровень оптимизации
# -funroll-loops: разворот циклов для уменьшения накладных расходов
# -flto: сквозная оптимизация (требует добавления -flto в LINKFLAGS)
# -fomit-frame-pointer: освобождает один регистр (не рекомендуется для отладочных сборок)
CCFLAGS      = $(HPL_DEFS) -O3 -march=sandybridge -mtune=sandybridge -mavx -funroll-loops -flto -fomit-frame-pointer
LINKER       = $(MPdir)/bin/mpif77
# ОПТИМИЗАЦИЯ: Добавляем -flto в флаги линковки для сквозной оптимизации
LINKFLAGS    = $(CCFLAGS)
ARCHIVER     = ar
ARFLAGS      = r
RANLIB       = echo
...
echo "Используется 16 процессов (8 ядер + HT)"
cd ~/hpl/bin/linux
